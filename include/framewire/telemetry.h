...
// upper bound on shader passes captured per frame. mpv lists one entry per pass
// in vo-passes, and a gpu shader chain stays well under 16 in practice
inline constexpr unsigned kMaxPasses = 16;
 ...
 *
 * The layout is pinned at 128 bytes so the ring buffer can find a slot with a
 * shift instead of a multiply, and so one record covers exactly two cache
 * lines. Every field is a fixed width integer and there are no pointers,
 * because the struct lives in shared memory that each process maps at a
 * different address.
 *
...
static_assert(sizeof(TelemetryRecord) == 128, "record size is part of the shm layout");
